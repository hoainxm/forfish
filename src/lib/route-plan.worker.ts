...
// KHÔNG dùng trực tiếp — đi qua planRouteWithDiagnosticsAsync (route-plan-async.ts).
...
/*  Trả nguyên OUTCOME (tuyến hoặc null + nhãn chẩn đoán) — object thuần nên
    structured clone được. Trả mỗi `plan` là mất luôn lý do "thiếu dự báo" /
    "chưa tìm được" mà thẻ dẫn đường cần để nói đúng với bà con. */
export type PlanResponse =
  | { id: number; ok: true; outcome: RoutePlanOutcome }
  | { id: number; ok: false };
...
ctx.onmessage = (e: MessageEvent<PlanRequest>) => {
  ...
  try {
    res = { id, ok: true, outcome: planRouteWithDiagnostics(args) };
  } catch {
