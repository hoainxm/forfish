...
/** Trạng thái bảo hành từ warrantyUntil (mirror getExpiryStatus). Ngày theo
 *  lịch VN qua lib/days.ts; hết bảo hành HÔM NAY = đã hết (đỏ). */
export function getWarrantyStatus(
  ...
  if (days === 0) {
    return { level: "expired", days, label: "Hết bảo hành hôm nay" };
  }
