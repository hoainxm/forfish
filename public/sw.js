...
/*  RÒ DỮ LIỆU CHÉO USER: kho /api/* khoá theo URL, KHÔNG theo user. Endpoint
    riêng tư (/api/me, /api/crew-reports, /api/admin…) dính chung khoá ⇒ user B
    mất sóng, SW trả bản cache của A (tên, serial, mã đơn, yêu cầu hỗ trợ). Chặn
    bằng ALLOWLIST `API_CACHE_ALLOW` bên dưới: chỉ dự báo/giá được cache, mọi
    /api khác KHÔNG nằm allowlist thì `fetch` handler `return` (SW ĐỨNG NGOÀI,
    client tự gọi mạng bằng timeoutSignal của mình) ⇒ không bao giờ lấy bản của
    người khác. KHÔNG dựng respondWith riêng cho path riêng tư: một `fetch(req)`
    trần trong respondWith TREO trên sóng "sống mà chết" (base invariant K1).
    (auth-scope.purgeApiCache còn nhờ SW xoá kho khi đổi user/logout — lớp dọn
    thêm.) */
...
/*
  ...
  Dọn nốt phản hồi riêng tư còn sót của user cũ — bổ trợ cho allowlist
  API_CACHE_ALLOW đã chặn cache path riêng tư từ đầu (bản cache cũ trước khi có
  chặn vẫn cần dọn).
*/
