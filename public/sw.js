...
self.addEventListener("fetch", (event) => {
  ...
  // API RIÊNG TƯ (đồ của tôi / sổ thuyền viên): mạng-thuần, KHÔNG cache. Mất
  // sóng → trả {ok:false, code:"offline"} (client lùi dữ liệu local) thay vì
  // bản cache của user khác. Chốt chống rò dữ liệu chéo tài khoản (đôi với luật
  // allowlist bên dưới — private API vừa không-cache vừa có tín hiệu offline rõ).
