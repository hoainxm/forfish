...
- **Tab Sản phẩm TÁCH ĐÔI, focus giới thiệu + upsale (user chốt 2026-06-11, hết kéo dài)**: chip tầng 1 **"Đang dùng"** (đồ đồng bộ + đồ tự ghi + nút thêm) / **"Của SDVICO"** (cửa hàng gọn `sdvico-catalog.tsx`): khu 1 = **"Mua thêm cho đồ đang dùng"** (dòng khách sở hữu, góc nhìn vật tư thay thế — chỗ upsale trúng nhất), khu 2 = lưới 2 CỘT các dòng còn lại; chạm dòng → BottomSheet chi tiết (blurb + món tiêu biểu `representativeProducts` — máy/thiết bị đinh lên đầu, có test — + nút Hỏi mua). KHÔNG đổ 9 thẻ full-width dọc nữa.
