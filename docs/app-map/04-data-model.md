...
### KHO BẢN TIN BÃO (vẽ đường đi) — migration [`0036_storm_bulletins.sql`](../../supabase/migrations/0036_storm_bulletins.sql) (2026-08-18) — ✅ ĐÃ APPLY prod
...
Quy ước: tính năng khóa MỚI → ~~bọc `components/login-gate.tsx` (UI)~~ dùng `PremiumLock`/`premium-gate.tsx` tại điểm chạm (07 §12 tầng 5; `login-gate.tsx` **ĐÃ XOÁ 2026-08-18 — gói E**, 0 importer; `PremiumLock` tự trả `null` khi `navigator.onLine === false`) **và** kiểm session ở API (thật). Hook trạng thái: `lib/use-auth.ts`. Khi Supabase chưa cấu hình (demo mode dev) thì KHÔNG khóa — giữ invariant demo mode §"Demo mode".
