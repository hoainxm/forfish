...
- **Client** `src/lib/heartbeat.ts` + luật thuần dùng chung hai phía `src/lib/heartbeat-policy.ts` — hàng rào offline: mất sóng thì KHÔNG gọi · `AbortSignal.timeout` + `.catch` nuốt sạch · gọi trong `useEffect` sau 3s, không `await` ở đường vẽ màn. Là POST nên service worker bỏ qua hẳn.
