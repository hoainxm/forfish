...
| LỊCH SỬ (giữ bản đã xoá) | migration [`0054_user_docs_history.sql`](../../supabase/migrations/0054_user_docs_history.sql) (2026-09-01) — bảng **chỉ-thêm** `user_docs_history`: mỗi lần client đẩy sổ lên, route chép BẢN CŨ sang đây TRƯỚC khi `upsert` đè. Cột: `id` (bigserial) · `owner_phone` · `kind` · `data` (bản CŨ) · `client_updated_at` (mốc của bản cũ) · `replaced_at` (giờ server — trục thời gian phân tích). Index `(owner_phone, kind, replaced_at desc)`. **Muốn biết bà con bỏ cái gì thì so bản cũ với bản mới** — `user_docs` giữ cả cuốn sổ là MỘT khối JSON nên không có dòng riêng cho từng việc để đánh dấu. Chủ dự án đã cân nhắc và LOẠI cách gắn `deletedAt` ngay trong sổ: nó làm sổ TRONG MÁY phình dần, mà máy chật chính là gốc của lỗi "việc đã xoá sống lại" vá cùng ngày. **RLS ĐÓNG HẲN** như bảng gốc — bảng này chứa cả CCCD lẫn metadata giấy tờ và GIỮ MÃI, tuyệt đối không để client đọc lại thứ bà con đã xoá. Chép lỗi thì KHÔNG chặn lượt đẩy (mất một dòng lịch sử còn hơn mất sổ của bà con). Đã khai ở [/quyen-rieng-tu](../../src/app/quyen-rieng-tu/page.tsx). |

- ⚠️ **CHƯA APPLY prod** (0050 + 0051 + 0054, ref `znzgugvfhgmiszqgjulk`, KHÔNG tự apply). Chưa apply → route trả 503/500, client giữ localStorage như cũ (không mất dữ liệu, chỉ chưa đồng bộ). Bucket `user-docs` có thể tạo tay ở tab Storage (Private) thay migration.
