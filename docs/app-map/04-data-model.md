...
- **Quên mật khẩu tự phục vụ (2026-07-21) — TÁI DÙNG quy trình sẵn có của CRM, KHÔNG xây mới**: SDFish auth là SĐT+mật khẩu, email ảo `{SĐT}@sdvico.local` không nhận thư ⇒ không làm được "gửi link đặt lại". Rà CRM thì phát hiện đã có TRỌN bộ: Edge Function `request-password-reset` (v23, 15/07) + bảng `password_reset_requests` (`user_id`→`users.id`, `verification_data`, `expires_at`, `processed_by`) + `approve-password-reset`/`reject-password-reset` + `password_reset_attempts` (chống dò theo IP) + cron dọn hết hạn. Đang dùng thật (6 duyệt · 3 hết hạn).
  ...
  - 🔴 **Chặn còn lại**: 46 account (45 customer + 1 sub) có `login_phone` nhưng **KHÔNG có bản ghi `users`** bên CRM → `request-password-reset` trả câu chung mà **KHÔNG tạo yêu cầu nào** ⇒ KH chờ mãi không ai gọi. Xử lý bằng Edge Function CRM `bulk-create-users-for-accounts` (hướng A, user duyệt 21/07).
- **Phủ tài khoản: mọi khách SDWork đều có login SDFish (yêu cầu user 2026-07-21)** — hiện **CHƯA đạt**. Số liệu CRM 21/07: khách thật (customer 574 · sub 106 · collaborator 5 · distributor 1 = **686**, loại `supplier` 22 vì là nhà cung cấp) → đủ điều kiện (có `login_phone`) **646**, SDFish đang có **630** từ CRM ⇒ **thiếu ~16**; thêm **40 khách thiếu SĐT** bị bỏ qua âm thầm. Ba lỗ hổng: (a) outbox gốc hardcode `type='customer'` ở 4 chỗ → nhóm sub/collab/distributor tạo mới sẽ KHÔNG có tài khoản dù đợt 30/06 đã tạo cho họ (luồng tự động HẸP HƠN provision) · (b) không có cơ chế bù khi webhook rớt · (c) thiếu `login_phone` = im lặng bỏ qua. Artifact vá: [go-live/sdwork-outbox-phu-tai-khoan.sql](../integration/go-live/sdwork-outbox-phu-tai-khoan.sql) 🔴 chưa apply (việc phía CRM).
