...
| Reset khi ĐĂNG XUẤT | Đăng xuất/gỡ máy xoá `USER_SCOPED_KEYS` (boats/crew/documents/…) PHẢI kèm `clearSyncMeta()` (`lib/user-sync`, 2026-09-18) reset `forfish.sync.v1` về 0 **cả bản trong bộ nhớ `metaCache`**. Không reset thì lần đăng nhập lại `syncAll` so LWW `server.clientUpdatedAt > metaOf().at` = **false** (mốc cũ còn ngang server) ⇒ KHÔNG kéo bản server ⇒ chủ thật thấy TRỐNG. ⚠️ 2026-09-18: 0050/0051 apply NHẦM project — đúng prod (znzgugvfhgmiszqgjulk) CHƯA có ⇒ hồ sơ tay CHƯA recoverable khi đăng nhập lại tới khi apply LẠI đúng. Khi đã apply đúng: boats/crew/documents/maintenance/materials kéo lại được; RIÊNG buyers/places/routes/cart KHÔNG đồng bộ nên xoá là mất hẳn (copy "Gỡ máy" chỉ hứa "phần đã đồng bộ"). |
