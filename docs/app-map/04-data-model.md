...
- **KỲ HẠN: 1 lần kích = 1 NĂM 6 THÁNG** (`PREMIUM_TERM_DAYS=548`/`nextPremiumUntil` trong `src/lib/tier.ts`, có test): còn hạn thì gia hạn CỘNG NỐI vào hạn cũ, hết hạn thì tính 1 năm 6 tháng từ hiện tại. Server tự tính — client không gửi hạn tay nữa. **Ưu đãi 2026-08-04** (chủ dự án chốt): gói 500k giữ giá nhưng nâng 1 năm → 1 năm 6 tháng (tặng 6 tháng ~ giá trị 300k); trước đó `PREMIUM_TERM_DAYS=365`. Ngoại lệ **300k = 1 năm** chỉ áp cho 2 SĐT cũ (`0384668304`, `0376002882`) — KHÔNG có luồng cấp 300k trong app. Bản +6 tháng đã chạy (log `granted_by='migration-0032-uu-dai-6thang'`), NHƯNG danh sách loại trừ khi đó ghi nhầm số thứ 2 → `0376002882` bị +6 tháng oan. Migration [`0032_premium_uu_dai_6thang`](../../supabase/migrations/0032_premium_uu_dai_6thang.sql) là bản **đối soát**: cắt `0376002882` về 1 năm (chốt chặn >13 tháng, idempotent) + **xoá 12 dòng log dấu migration** để trả credit "người cấp" (màn gom theo `granted_by` lượt cấp gần nhất) về staff thật — không đụng `premium_until` đã cộng.
