...
| `tide-stations.v1.json` (5,6 KB) | **4 trạm ĐO** (UHSLC gauge) + **7 trạm MÔ HÌNH EOT20** (DGFI-TUM, SEANOE **CC-BY 4.0** — thương mại được) | **4 → 11 trạm** | 4 trạm đo giữ nguyên; 7 trạm mô hình gắn cờ `nguon=model` (RMSE 0,128 m), phủ thêm **bờ Tây (Rạch Giá/Cà Mau — vịnh Thái Lan)** trước đây trắng. `tides.ts` thêm `isModelStation`/`tideModelCaveat`/`tideTrustText(distanceKm, station?)`; `fishing-map-view` gọi `tideTrustText(gan.distanceKm, gan.station)` để trạm mô hình **luôn hiện "ước tính"** kể cả khi gần (honesty — không đội bản đo lên bản mô hình) |
