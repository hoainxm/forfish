...
(máy dùng chung ở bến, hay máy vừa cài lại, là thấy việc của người khác). Đã đăng nhập thì mọi thứ y như
cũ: vẫn gom MỌI tàu, vẫn gắn `boatLabel` khi >1 tàu (AC-7), vẫn sort đỏ→vàng theo ngày thật, phân loại
cố-định/động (R1/R2), cascade R3, guard R7 không bị chạm. AC-1..7 không đổi một dòng. -->
<!-- re-verified: 2026-08-29 — đợt rà UI toàn app (07 §5b) chạm 6/7 file covers (boat-switcher, document-vault, maintenance-reminders, crew-list, boat-products, urgent-strip) nhưng KHÔNG đổi một AC đa-tàu nào. Đã đối chiếu từng file: phân loại cố-định-theo-tàu vs động-theo-chủ (R1/R2) giữ nguyên — thuyền viên vẫn KHÔNG mang `boatId`, giấy tờ/bảo dưỡng/sản phẩm vẫn gắn `boatId`; cascade R3 (`removeBoat` ghi danh sách tàu TRƯỚC rồi mới purge) và guard R7 (luôn còn ≥1 tàu) không bị chạm; `addBoat`/`updateBoat`/`removeBoat` giữ nguyên chữ ký và nhánh khoá-cửa-ghi K4; AC-1..7 không đổi. Thứ đổi đều là HÌNH: nút hành động về khuôn `SQ_BTN` ô vuông inline thay cho dải full-width, hàng theo khuôn [thân flex-1] + [ô w-16], form thu về CHỈ BÀY CÁI KEY (DocumentForm 5 ô → 2, CrewForm 6 nhóm → 3, ProductForm 5 → 2, MaintenanceForm bỏ ô số chu kỳ vì nó và 4 chip là HAI ĐƯỜNG cho cùng một giá trị — chip "Khác" mở lại ô đó khi cần). BA chỗ đổi HÀNH VI, đều là bịt lỗ chứ không sửa AC: (1) `crew-list` nút "Cảnh báo" trên người chưa có CCCD/SĐT nay VÔ HIỆU thay vì mở sheet "Sửa thông tin" — một nút hai việc, nhãn hứa việc nó không làm; lý do nằm trong thân hàng. (2) `urgent-strip` "Xem hết" từ `setExpanded(true)` một chiều thành toggle — trước xoè rồi hết đường thu. (3) `document-photos` (ngoài covers) xoá ảnh giấy tờ nay qua ConfirmDialog. Không đụng khoá `forfish.boats.*`, không thêm request mạng. -->
<!-- re-verified: 2026-08-25 — maintenance-reminders.tsx BỎ ô empty-state "Chưa có việc bảo dưỡng nào · bấm nút cam" (user chốt; nút Thêm đã mời sẵn), NHƯNG GIỮ NGUYÊN nhánh ĐỌC-HỎNG "Chưa mở được lịch bảo dưỡng · việc cũ chưa mất" (đổi điều kiện `sorted.length===0` → `sorted.length===0 && readFailed`). Đây là thay đổi HOW-nhìn thuần: dữ liệu per-tàu (`boatId`), loader 3 nhánh + KHOÁ CỬA GHI khi đọc hỏng (K4), phân loại cố-định/động (R1/R2), cascade R3, guard R7 giữ nguyên. AC-1..7 không đổi. -->
<!-- re-verified: 2026-08-21 — boat-switcher.tsx CHỈ THÊM `export` cho `BoatForm` (để vms-renewal.tsx tái dùng form thêm/sửa tàu cho luồng gia hạn VMS — câu trả lời user #2 "dẫn tới chỗ thêm mã tàu"). 0 đổi hành vi đa-tàu: BoatForm không đổi props/logic; addBoat/updateBoat/removeBoat, phân loại cố-định/động (R1/R2), cascade R3, guard R7 giữ nguyên. AC-1..7 không đổi. -->
<!-- re-verified: 2026-08-18b — đợt "audit thông báo" (ops/audit-notify-2026-08-18) chạm 6/7 file covers nhưng KHÔNG đổi một AC đa-tàu nào (R1/R2 phân loại, R3 cascade, R6/AC-7 nhãn tàu trên dải khẩn, R7 ≥1 tàu). Cụ thể: `urgent-strip.tsx` vẫn gom MỌI tàu + gắn `boatLabel` khi >1 tàu (giữ AC-7), đổi: đọc SDVICO qua `useSdvicoAssets` (không fetch riêng), sort đỏ→vàng theo ngày thật (bỏ -9999), nợ SDVICO chỉ vào dải khi QUÁ HẠN, ngày qua `useTodayVN`, loader `readUserList` (rác = rỗng); `boat-switcher.tsx` alert "máy hết chỗ" chuyển ra NGOÀI sheet (dưới chip tàu) để thấy được ngay sau khi form đóng — cascade R3/guard R7 không đổi; `document-vault`/`crew-list`/`maintenance-reminders`/`boat-products`: loader 3 nhánh + ghi CHỈ khi thao tác (không ghi sau hydrate), thẻ MẪU đeo băng neutral "Ví dụ:", ngưỡng dùng `lib/days.ts` (SOON_DAYS_DOCS=30/SERVICE=14, days===0 = đỏ) — dữ liệu per-tàu (`boatId`) và thuyền viên động-theo-chủ giữ nguyên. Nợ kỹ thuật ghi trong file: urgent-strip còn 4 dòng chép getDueStatus của maintenance (tách sang lib/maintenance.ts sau). -->
...
ttl_days: 90
<!-- DOC-STATUS: SUSPECT (2026-09-01) — code 'src/components/urgent-strip.tsx' doi sau last_verified. DOI CHIEU VOI CODE truoc khi tin. May quan ly dong nay, dung sua tay. -->
