...
| `/api/fish-forecast` | 1 (API) | `src/app/api/fish-forecast/route.ts` | **DỰ BÁO CÁ (PFZ)**: kéo lưới SST + phù du NOAA ERDDAP, chấm điểm loài qua `lib/fish-predict.ts`, cache 6h. Fail → `{ok:false}`, client lùi về mùa vụ |
...
src/
  ...
  lib/
    ...
    fish-predict.ts     # Trục 1: DỰ BÁO CÁ (PFZ) — khẩu vị 11 loài (trapezoid SST + dải chl + trọng số 4 yếu tố) × habitat{mồi, front nhiệt, front mồi, rìa xoáy SSHA} × mùa vụ/vùng; gradientStrength/buildFishForecast(sst,chl,sla,month) thuần có test; SSHA tuỳ chọn (fail→chia lại trọng số); client /api/fish-forecast
