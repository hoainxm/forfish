...
| `/api/tiles/contour/[z]/[x]/[y]` | 1 (API) | `src/app/api/tiles/contour/[z]/[x]/[y]/route.ts` | Proxy contour OpenSeaMap depth WMS — **CHƯA CÓ LỚP NÀO GỌI** (2026-07-25 dò lại: route + upstream đều sống, trả PNG contour 250/500/750m thật). Chưa nối vì trùng vạch với đường đẳng sâu tự sinh (`isobaths.v1.json`) ở đúng dải zoom bà con dùng → rối bản đồ. Giữ/bỏ chờ review |
...
src/
  ...
  components/
    ...
    fishing-map-view.tsx # Trục 1: MÀN HÌNH map-first — map full-screen + RaKhoiControls (rail phải) + SnapSheet đáy (vuốt 3 nấc); nhãn chủ quyền + ranh giới + bão; công cụ đo khoảng cách 2 điểm; toạ độ/khoảng cách theo lib/map-prefs. Badge TUỔI LỚP CÁ (lib/fish-age — "Bản đồ cá — Ảnh ngày … · lấy về …", nền warn khi cũ) + chọn khung ngày lưới nói thật khi máy chưa lưu khung đó (savedGridDays)
    ra-khoi-controls.tsx # Trục 1: RAIL PHẢI 6 nút (Hải đồ/Ngư trường/Thời tiết/Điểm đã lưu/Công cụ/Cài đặt) — mỗi nút mở 1 panel; nơi DUY NHẤT bật/tắt-chọn lớp hiện trên map (Phương án A). Chọn loài + quản lý điểm = drill-down/nhúng trong panel (không bottom-sheet). Kèm thẻ CHUẨN BỊ ĐI BIỂN neo mép trái (lib/pretrip): 1 nút to + thanh tiến trình + dòng thường trực "Trong máy: dự báo tới DD/MM · N chỗ"; ẩn khi mở panel / thu rail
  ...
  lib/
    ...
    pretrip.ts          # Trục 1: "CHUẨN BỊ ĐI BIỂN" — tải sẵn trước khi rời bờ, KHÔNG nguồn mới: gọi lại fetchSeaPoint (mỗi chỗ ghim + chỗ đang xem, dedupe ô 0,25°) + fetchFishForecast + fetchForecastGrid các khung PRETRIP_GRID_DAYS [3,7,16], chạy TUẦN TỰ có tiến trình. savedSummary/savedLine ("Trong máy: dự báo tới DD/MM · N chỗ") + doneLine (thuần, có test)
    fish-age.ts         # Trục 1: TUỔI BẢN ĐỒ CÁ — tách `date` (ngày ẢNH vệ tinh, trễ ~2 ngày) với `generatedAt` (lúc máy chủ TÍNH = lúc lấy về); fishForecastAge(f, nowMs) → {imageDays, fetchedAtMs, warn, label} (thuần, có test). warn khi ảnh cũ hơn FISH_STALE_DAYS=3 → UI đổi nền warn thay vì để bản 12 ngày tuổi trông như bản mới
