...
| `/api/auth/otp/request` + `/api/auth/otp/verify` | AUTH (API) — **OTP chính (Đợt 1, 2026-06-16)** | `src/app/api/auth/otp/{request,verify}/route.ts` | SĐT → mã (lưu hash `otp_codes`, gửi qua `lib/otp/provider` adapter — provider `stub`, cắm Zalo/SMS sau, rate-limit 60s) → verify → admin `generateLink` magiclink → client `verifyOtp`. Chưa cấu hình env/DB → 503, UI lùi mật khẩu. Logic thuần `lib/otp/codes.ts` có test |
| `/api/sdwork/webhook` | INGEST (API) — **Đợt 1** | `src/app/api/sdwork/webhook/route.ts` | SDWork đẩy KH/thiết bị/vật tư → verify HMAC (`x-sdwork-signature` + `SDWORK_WEBHOOK_SECRET`) trên raw body → upsert bảng SDFish (admin/service-role). Map thuần `lib/sdwork-webhook.ts` (idempotent `sdwork_ref`) có test. Xem [04 §5b](04-data-model.md) |
| `/api/auth/sso` + `/api/auth/signup` | AUTH (API) — ⚠️ mật khẩu/SSO PHỤ, chuyển tiếp (retire sau khi webhook ổn) | `src/app/api/auth/{sso,signup}/route.ts` | Route MỎNG → Edge Function `auth-gateway` trong project ForFish (qua `lib/auth-gateway.ts`). **sso**: verify SĐT+mk với CRM SDViCo → đồng bộ mk vào tài khoản ForFish → client `signInWithPassword` chuẩn (đã bỏ magic-link, không cần `SUPABASE_SERVICE_ROLE_KEY`). **signup**: tạo user email-ảo `{SĐT}@sdvico.local` ĐÃ confirm (email ảo không có hòm thư để bấm link). Env cần: chỉ `NEXT_PUBLIC_SUPABASE_URL/ANON_KEY` (đã set local + Vercel production 2026-06-10) |
...
src/
  ...
  lib/
    ...
    phone.ts            # AUTH: helper SĐT VN THUẦN (normalizeVnPhone/phoneToEmail/isValidVnPhone) — dùng cả server (route OTP/webhook); auth-form.tsx re-export
    otp/codes.ts        # AUTH Đợt 1: OTP thuần (sinh/hash/verify/hạn/rate-limit) — có test
    otp/provider.ts     # AUTH Đợt 1: adapter gửi OTP (provider `stub`, cắm Zalo/SMS sau)
    sdwork-webhook.ts   # INGEST Đợt 1: verify HMAC + map payload SDWork→bảng SDFish (thuần, có test)
    supabase/admin.ts   # SERVICE-ROLE client (server-only, bypass RLS) — chỉ webhook + OTP verify; null khi chưa cấu hình
