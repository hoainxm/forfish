...
| `/api/auth/exists` | AUTH (API) — **2026-07-21** | `src/app/api/auth/exists/route.ts` | SĐT đã có tài khoản SDFish chưa — CHỈ boolean. Service-role TÁI DÙNG RPC `auth_user_id_by_phone` (0003, đã prod) — không DDL mới. SĐT đi trong BODY (không URL). ⚠️ user-enumeration chấp nhận có chủ đích (username=SĐT vốn đoán được) — route là chokepoint gắn rate-limit sau, lộ trình [ADR 0002](../adr/0002-siet-bao-mat-sau-sd123456.md). Demo mode/lỗi → `ok:false`, login quay về câu gộp |
...
src/
  ...
  components/
    ...
    debt-ledger.tsx     # Trục TIỀN: SỔ CÔNG NỢ ĐA ĐỐI TƯỢNG — chủ nợ (đại lý dầu/nậu/ngân hàng) + lịch sử vay/trả + dư nợ (← lib/debts), CRUD localStorage forfish.debts.v1 (KHÔNG seed demo, rỗng khi chưa có); tab Công nợ /tien, khóa LoginGate
