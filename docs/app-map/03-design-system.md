...
<!-- re-verified: 2026-08-29 — DOCK iOS: GẮN CỔNG PHIÊN BẢN cho workaround `--app-vh` (viewport-gap-fix.tsx). Apple ĐÃ SỬA bug viewport standalone (WebKit 158055568) ở Safari 26.1. Chạy workaround trên iOS ≥26.1 latch `--app-vh` theo tab CAO (cuộn được) rồi đè tab THẤP (Ra khơi — map fixed) → khung cao quá màn → dock CỤT + giá trị nhảy qua lại 2 tab ("không lưu"); user máy iOS 26.6 dính đúng ca này. NAY: `viewportBugFixed()` đọc UA (OS ≥26.1) → iOS đã-sửa KHÔNG gắn class `pwa-frame` → CSS về NHÁNH GỐC (dock `fixed bottom:0` + `100dvh`/`--dock-total`, native xử lý đúng per-tab); chỉ iOS standalone <26.1 (còn dính bug) mới chạy workaround `--app-vh`. KHÔNG đổi token/CSS — chỉ đổi ĐIỀU KIỆN chạy JS. -->
