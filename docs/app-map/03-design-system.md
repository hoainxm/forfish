...
- **Lớp Dự báo cá = heatmap XANH LÁ + hồng tâm (2026-06-10, tham khảo cách thể hiện PFZ của OceanFishMap)**: ô điểm → lớp `heatmap` maplibre (ramp rgba xanh lá `#95d5b2→#1b4b2c`, weight theo điểm 35→100, radius theo zoom phủ kín lưới 0.25°) — vùng mềm tự nhiên, KHÔNG vẽ ô vuông. ≤8 điểm nóng nhất (≥75 điểm, cách nhau ≥0.7°) vẽ marker **hồng tâm** `TargetIcon` (xanh đậm trên đĩa trắng) — chạm là bay tới + xem dự báo chỗ đó. Thẻ cá trong sheet kèm số môi trường ("Nước 28°C · mồi vừa") + **tuần trăng** (`lib/moon.ts`). Phân màu: XANH LÁ = riêng lớp cá; cam-đỏ = riêng ranh giới; xanh dương = tuyến đi.
- **Chọn loài cá**: hàng chip nổi trên map (Mọi loài + từng loài đang vụ, loài mạnh nhất xếp trước) — chọn loài thì ô tô màu CHỈ theo loài đó; nhãn loài rải theo vùng đã bỏ (rối). Chi tiết loài tại điểm chạm nằm trong sheet.
