...
| R2 | "đã đối chiếu" CHỈ do SDWork xác nhận, SDFish không tự đặt | SDWork offline → cờ giữ "chờ đối chiếu", không tự khớp |
...
- 2026-07-30 — **BUILD đợt 2 (NV3)**: bảng `payments` (0026, MÃ CK + reconciled_status, KHÔNG số tiền), PATCH `action='record_payment'` (staff, manager scoped R3), GET đính trạng thái thu mới nhất, UI chip trạng thái thu + dialog nhập mã CK ở /quan-tri. AC-3/4 có mã. **CÒN**: NV4 trace outbound bắn SDWork + NV5 webhook SDWork xác nhận (cần SDWork dựng đầu nhận) + NV7 audit — đợt sau.
