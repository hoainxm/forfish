...
|---|---|---|---|---|
| Chuyến biển | sổ lãi/lỗ + báo cáo năm | — | drawer (≤5 field) | trong /tien tab Hiệu quả (3 chip: Sổ lãi/lỗ · Báo cáo năm · Chia tiền) |
...
→ 7 object, đúng trần. Tạo/sửa đều ≤5 field nên dùng **drawer/bottom-sheet**, KHÔNG đẻ page riêng (quyết định đã chốt).
...
- **Tab trong page**: /tau = 4 tab (Giấy tờ/Dịch vụ/Sản phẩm/Mức phạt); /tien = 2 tab (Giao dịch/Hiệu quả). Trong tab dùng `ChipRow` (≤3 tier: Tabs → chip1 → chip2). KHÔNG tab lồng tab.
...
| 3 | Tàu `/tau` | dock, nhắc `?tab=` | Giữ tàu đủ điều kiện + đồ SDVICO | xử việc đỏ (nợ/quá hạn) | theo tab | M |
