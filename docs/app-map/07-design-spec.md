...
| 7 | Đăng nhập `/login` | chip hero, gate | Vào bằng SĐT — **OTP chính** (gửi mã → nhập), mật khẩu phụ | (vào app) | "Gửi mã đăng nhập" | L |
...
| Đăng nhập | — | — | OTP sai/hết hạn "Mã không đúng…"; chưa cắm provider → lùi mật khẩu; mật khẩu sai "Sai SĐT/mật khẩu" | "Đang gửi…" / "Đang vào…" |
...
<!-- re-verified: 2026-06-16 — /login đổi sang OTP chính (SĐT→mã) + mật khẩu phụ (Đợt 1 auth riêng); nav/screen map/object model không đổi -->
