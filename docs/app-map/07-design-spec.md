...
- **Ghi dữ liệu bà con tự nhập KHÔNG được nuốt lỗi** (2026-07-31, giữ từ base): `saveUserJson` (lib/user-store.ts) trả boolean; hỏng thì banner ĐỎ "Máy hết chỗ — CHƯA lưu được …". Trước đây catch rỗng: màn hình hiện đúng thứ vừa nhập (trong bộ nhớ) mà máy chẳng giữ gì, mở lại app là mất trắng.
...
- **Bổ sung tính năng mới 2026-07-27** (đồng bộ `lib/tour.ts` + `guide-content.mjs`): tour Ra khơi thêm 2 bước `target:null` — **Tin bão Biển Đông** và **Dẫn đường + dự báo cá** (nay đủ 6 bước = trần). Sách `huong-dan.html`: màn Ra khơi thêm khối `note` (bão / dẫn đường tiết kiệm dầu / dự báo cá + thời tiết >3 ngày là **premium** / xem bản đồ đã lưu khi mất sóng) và 2 mục hỏi-đáp về premium + cập nhật mục "mất sóng". Các mục premium/bão/route là overlay có-điều-kiện → đưa vào **prose/note**, KHÔNG numbered mark (tránh "KHÔNG THẤY" lúc chụp). Đồng thời sửa mark cũ `tai-khoan`: bỏ "Xem hướng dẫn đầy đủ" (đã gỡ 2026-07-25) → thay bằng công tắc "Chỉ dẫn trên màn hình".
