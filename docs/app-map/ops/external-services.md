...
| **NOAA ERDDAP** | SST / phù du / front (dự báo cá) | Không key | `lib/fish-predict.ts`, `app/api/fish-forecast` | cache 6h, **server timeout 20s/lưới** (vài MB), **client fetch 25s** | Lớp cá pill đỏ "chạm để thử lại"; lùi mùa vụ; nguồn treo → route fail-fast `{ok:false}`, KHÔNG treo serverless |
| **HYCOM** (OPeNDAP) | Tầng nhiệt D20 (cá ngừ) | Không key | `lib/hycom.ts` | fetch song song ERDDAP, **timeout 20s** + `.catch→null` | Chia lại trọng số habitat, không D20 vẫn ra cá; treo → null (không treo `await thermoP`) |
