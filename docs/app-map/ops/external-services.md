...
<!-- re-verified: 2026-07-28 — THÊM nguồn ERDDAP LỊCH SỬ (chỉ dùng OFFLINE, không đụng runtime): CoralTemp `noaacrwsstDaily` (1985→nay) + chl THÁNG `noaacwNPPVIIRSSQchlaMonthly` (2012→nay) dựng bản đồ MÙA VỤ 12 tháng (public/data/fish-climatology.v1.json, 69×65 ô 0,25°, ~71 KB, SW pre-cache sdfish-v6) qua scripts/collect-fish-climatology.mjs; scripts/fit-fish-blend-weights.mjs backtest 12 mốc gốc × 7 tầm ngày ra tỷ lệ pha trộn w(d). ĐO THẬT: w giảm 0,793 (d1) → 0,493 (d16) tự nhiên đơn điệu, blend THẮNG persistence ở MỌI tầm khi kiểm chéo (+4,8% → +10,5% RMSE), guard always-on-term PASS (biên độ w 0,30). KHÔNG route mới, KHÔNG đổi payload /api/fish-forecast, runtime KHÔNG gọi thêm nguồn nào. -->
