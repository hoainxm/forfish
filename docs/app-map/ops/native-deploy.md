...
- **PWA cài được**: `src/app/manifest.ts` (→ `/manifest.webmanifest`), `public/sw.js` (service worker offline shell), `src/components/sw-register.tsx` (đăng ký SW, chỉ production), icon `brand/logo-sdfish.png` (bộ logo gốc — không commit SVG tay) → `public/icons/*`, **Android launcher `android/.../mipmap-*/ic_launcher{,_round,_foreground}.png`** **và AppIcon iOS `ios/App/App/Assets.xcassets/AppIcon.appiconset/AppIcon-512@2x.png`** (1024², KHÔNG alpha, KHÔNG bo — iOS tự mask; chỉ sinh khi đã `cap add ios`) — tất cả sinh bằng `npm run icons`, devDep `sharp`; auto crop bỏ chữ "SDFish", pad mark vào nền trắng bo 22% (iOS thì không bo). ⚠️ Bắt buộc chạy lại `npm run icons` sau `cap add android` để đè icon Capacitor mặc định (chữ X) — nếu không app cài về khác store listing → Google flag "Misleading Claims / listing mismatch". `layout.tsx` khai `manifest` + `icons` + `appleWebApp` + `viewport.themeColor`.
...
- **CI auto Android (2026-07-28)**: `.github/workflows/android-release.yml` — build AAB đã ký + upload CH Play qua Gradle Play Publisher. Trigger tay hoặc tag `vX.Y.Z`; `versionCode` tự tăng theo giờ UTC (`date -u +%y%j%H%M`, không dùng run_number — xem rà soát Actions 2026-09-04). Cần secrets keystore + `PLAY_SERVICE_ACCOUNT_JSON`. Chi tiết [build-publish-store.md §3b](build-publish-store.md). iOS chưa auto (cần Mac/Xcode).
