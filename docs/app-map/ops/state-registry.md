...
| `forfish.heartbeat.retry.v1` | **Sớm nhất được thử lại** nhịp "đã mở app" (epoch ms, mốc TUYỆT ĐỐI). Không nghe được máy chủ → nấc kế trong thang **3 phút → 5 phút → 12 giờ**; máy chủ CÓ trả lời mà chưa ghi được → `now+30phút`; ghi được → xoá | `lib/heartbeat.ts` | chính nó | 1 | xoá → lần mở app kế tiếp thử lại một nhịp. Không có PII |
